...
const orderModel = {
  findOrdersByCustomerId: (customerId) => {
    return new Promise((resolve, reject) => {
      cn.query(
        `SELECT orders.*, products.product_name, products.listing_price,
          order_items.product_quantity, order_items.item_price
        FROM orders
  ...
  createForCustomer: (customerId, orderData) => {
    ...
    return new Promise((resolve, reject) => {
      cn.beginTransaction((err) => {
        ...
        cn.query(
          `INSERT INTO orders (customer_id, shipping_address_id, shipping_method_id, total_price, order_date, order_status)
          VALUES (?, ?, ?, ?, ?, ?)`,
          ...
          (err, result) => {
            ...
            // Insert items sequentially to handle errors
            const insertItem = (index) => {
              ...
              cn.query(
                `INSERT INTO order_items (order_id, product_id, product_quantity, item_price)
                VALUES (?, ?, ?, ?)`,
